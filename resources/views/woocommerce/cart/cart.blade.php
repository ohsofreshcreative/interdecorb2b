	...
	<div class="grid grid-cols-1 md:grid-cols-[2fr_1fr] gap-10 mt-6">
	  <div class="">
		<form class="woocommerce-cart-form" action="{{ esc_url( wc_get_cart_url() ) }}" method="post">
		  ...
		  <table class="shop_table shop_table_responsive cart woocommerce-cart-form__contents bg-white !border !border-primary !rounded-3xl overflow-hidden" cellspacing="0">
			<thead>
			  <tr>
			...
			<tbody>
			  ...
			  @foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item)
				...
				@if ($_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters('woocommerce_cart_item_visible', true, $cart_item, $cart_item_key))
				  ...
				  @endphp
				  <tr class="woocommerce-cart-form__cart-item odd:bg-primary-50/30 {{ esc_attr(apply_filters('woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key)) }}">
	  ...
	  <div class="w-auto">
		...
		<div class="cart-collaterals border border-primary rounded-3xl overflow-hidden">
		  @php
			/**
