...
<?php if ($has_orders) : ?>
	<div class="bg-white p-6 rounded-xl shadow-sm">
		...
		<div class="flex justify-between items-center mb-4">
			<h6 class="text-h7 font-bold">Ostatnie zamówienia</h6>
			<a href="<?php echo esc_url(wc_get_endpoint_url('orders')); ?>" class="text-sm !underline hover:underline">Zobacz wszystkie</a>
		</div>
		<table class="woocommerce-orders-table woocommerce-MyAccount-orders shop_table shop_table_responsive my_account_orders account-orders-table">
			...
			<tbody>
				<?php
				$orders_to_iterate = is_object($customer_orders) && isset($customer_orders->orders) ? $customer_orders->orders : $customer_orders;
				foreach ($orders_to_iterate as $customer_order) {
